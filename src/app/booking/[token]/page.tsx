...
function DobInput({ value, onChange, required }: { value: string; onChange: (v: string) => void; required?: boolean }) {
  ...
  return (
    <div
      ...
      className="flex items-center w-full rounded border border-mesa-accent/40 bg-brown-900 text-sm text-white focus-within:border-mesa-accent pl-3"
    >
...
export default function ManageBooking({
  ...
  function collapseReschedulePlayer(i: number) {
    setExpandedPlayers((s) => {
      if (!s.has(i)) return s;
      const next = new Set(s);
      next.delete(i);
                ...
                <div>
                  ...
                  {reschedulePlayers.map((player, i) => {
                    ...
                    <div key={i} className="flex flex-col gap-2 pb-2">
                      {isExpanded ? (
                        <div
                          onClick={() => collapseReschedulePlayer(i)}
                        ...
                        >
                          ...
                          <div className="space-y-2">
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                              <div>
                                ...
                                <select required value={player.grade} onChange={e => setReschedulePlayers(prev => prev.map((p, j) => j === i ? { ...p, grade: e.target.value } : p))} onClick={(e) => e.stopPropagation()} className="w-full rounded-lg border border-mesa-accent/40 bg-brown-800 px-3 py-2 text-white text-sm focus:border-mesa-accent focus:outline-none">
                          ...
                          {player.name && (
                            <div className="flex justify-end pt-1">
                              <button
                                type="button"
                                onClick={(e) => { e.stopPropagation(); toggleReschedulePlayerExpanded(i); }}
